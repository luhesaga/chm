...
export class MatricularEstudiantesCarreraComponent implements OnInit, AfterViewInit, OnDestroy  {
  ...
  obtenerUsuarios()
  {
    const unsubscribe = this.userService.listUsers()
    .valueChanges()
    .subscribe(users => {
      this.users = users;
  ...
  obtenerMatriculados()
  {
    const unsubscribe = this.carrerasService.matriculadosObtener(this.dataReceived.idCarrera)
    .valueChanges()
  ...
  obtenerDesmatriculados(matriculados:any[])
  {
    const desmatriculados = this.users.filter(usuario =>{
      const encontrado = matriculados.findIndex(matriculado => usuario.id === matriculado.id);
      if(encontrado === -1)
      {
  ...
  datosDelArrayDeTabla(desmatriculados:any[])
  {
  ...
  inputMesesMatricula(element:any, tipoMatricula:string, mesMatricula?:number)
  {
    Swal.fire({
      title: 'Meses',
      input: 'number',
    ...
    .then((confirm)=>{
      ...
      {
        this.usuarioMatriculaCursoIndividual(element, tipoMatricula, Number(confirm.value));
      }
    });
  }

  usuarioMatriculaCursoIndividual(element:any, tipoMatricula:string, mesMatricula?:number)
  {
    this.mensajeMatriculandoEstudiante(`${element.nombres} ${element.apellidos}`);
    ...
    this.cursosCarrera.forEach((curso, index) => {
      const usuarioCurso = this.courseService.listCoursesByUser(curso.id, element.id)
      .valueChanges()
      .subscribe(usuario =>{
        if(usuario)
        {
        ...
        const numeroUsuarios = element.matriculaIndividual.length + element.cursosNoMatriculados.length;
        if(numeroUsuarios === ultimoCursosCarrera)
        {
          ...
          element.fechaFinalizacionMatricula= matricula.fechaFinalizacionMatricula;
          element.tipoMatricula = matricula.tipoMatricula
          this.matricularEstudiante(element);
        }
        usuarioCurso.unsubscribe();
      }, ()=>this.mensajeError(`error al buscar los cursos de ${element.nombres} ${element.apellidos}`));
    });
  }

  tipoMatricula(tipoMatricula:string, mesMatricula?:number)
  {
    if(tipoMatricula === 'indefinida')
    {
      ...
      fechaFinalizacionMatricula.setMonth(fechaFinalizacionMatricula.getMonth()+ mesMatricula)
      return {
        fechaMatricula: new Date(),
  ...
  matricularEstudiante(element:any)
  {
    this.carrerasService.matricularUsuario(element, this.dataReceived.idCarrera)
    .then(()=>{
    ...
    .catch(()=>{this.mensajeError('No se pudo matricular al usuario, por favor intente otra vez');});
  }

  matricularAlosQueNoEstanEnLosCursosDeCarrera(element:any)
  {
    ...
    if(ultimoCurso !== -1)
    {
      element.cursosNoMatriculados.forEach(async (curso:any, index)=>{
        await this.courseService.registerUserToCourse(element, curso.id, element.id)
        .then(()=> {
        ...
        ()=>{this.mensajeError(`No se pudo matricular en el curso ${curso.nombre}`);})
        .catch(()=>{this.mensajeError(`No se pudo matricular en el curso ${curso.nombre}`);});
      })
    }
    else
    {
  ...
  cambiarEstadoMatriculaDeEstudiantesCursos(element:any)
  {
    const ultimoCurso = element.matriculaIndividual.length - 1;
    element.matriculaIndividual.forEach(async (curso:any, index) =>
      {
        if(curso.tipoMatricula === 'mes' && element.tipoMatricula==='indefinida')
        {
          await this.courseService.updateUserOfCourse(element, curso.cursoId, element.id)
          .then(()=>{},
          ()=>{
            this.mensajeError(`No se pudo actualizar la matricula de ${element.stdName}`);
          })
          .catch(()=>{this.mensajeError(`No se pudo actualizar la matricula de ${element.stdName}`);});
        }else if(curso.tipoMatricula === 'mes' && element.tipoMatricula==='mes')
        {
          if(element.fechaFinalizacionMatricula > curso.fechaFinalizacionMatricula.toDate())
          {
            await this.courseService.updateUserOfCourse(element, curso.cursoId, element.id)
            .then(()=>{},
            ()=>{
  ...
  mensajeMatriculandoEstudiante(nombre:string)
  {
    Swal.fire({
