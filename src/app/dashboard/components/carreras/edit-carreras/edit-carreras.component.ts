...
export class EditCarrerasComponent implements OnInit {
  ...
  obtenerCarrera()
  {
    const unsubscribeCarrera = this.carrerasService.obtenerCarrera(this.idCarreras)
    .valueChanges()
  ...
  uploadNewImage()
  {
    ...
    const fileRef = this.fireStorage.ref(`carreras/${this.idCarreras}/${nameImage}`);
    this.fireStorage.upload(`carreras/${this.idCarreras}/${nameImage}`,this.imageField.value)
    .then(()=>{
  ...
  {
    this.carrerasService.actualizarCarreras(this.formCarreras.value, id)
